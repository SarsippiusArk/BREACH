...
export function drawHUD(ctx, state) {
  const { p1, p2, score, hiScore, bossHp, bossMaxHp } = state;
...
function drawPilotStatus(ctx, p, x, y, rightAlign) {
  if (!p) return;
  ...
  for (let i = 0; i < Math.min(lives, 5); i++) {
    drawMiniShip(ctx, lifeX + i * 10, y, pilotColor || COL.ACCENT);
  }
